...
import { RetryExecutor, ErrorMatchers } from '../../src/resilience/retryExecutor';
...
test('RetryExecutor should not retry non-retryable errors', async () => {
  let attemptCount = 0;
  const retryExecutor = new RetryExecutor({ maxAttempts: 3, baseDelay: 10, maxDelay: 100, backoffMultiplier: 2, jitter: false, retryableErrors: ['RetryableError'] });
...
test('RetryExecutor should respect max delay', async () => {
  const retryExecutor = new RetryExecutor({ maxAttempts: 3, baseDelay: 1000, maxDelay: 100, backoffMultiplier: 2, jitter: false, retryableErrors: ['TestError'] });
  ...
  // Should have waited approximately 100ms + 100ms = 200ms (capped at maxDelay)
  assert.ok(duration >= 190 && duration <= 250);
  assert.strictEqual(attemptCount, 3);
});
