...
/**
 * Message types for the processor
 */
type ProcessorMessage =
  | { type: 'process'; id: string; data: Record<string, any> }
  | { type: 'shutdown' };
 ...
 */
class WorkerActor extends TypeSafeActor<ProcessorStats, ProcessorMessage> {
  private circuitBreaker: CircuitBreaker;
  private metricsWindow: RollingWindow;

  constructor(
    id: string,
    initialState: ProcessorStats,
  ...
  private processMessage(_message: {
    type: 'process';
    id: string;
...
/**
 * Dispatcher Actor - distributes work to worker actors
 */
type DispatcherMessage =
  | { type: 'submit'; id: string; data: Record<string, any> }
  | { type: 'getMetrics' };
...
class DispatcherActor extends TypeSafeActor<DispatcherState, DispatcherMessage> {
  private workerIndex = 0;
  private timeTracker: TimeWindowTracker;
...
async function runExample(): Promise<void> {
  ...
  const dispatcherRef = system.getActor('dispatcher') as any;
  ...
  for (const workerId of workerIds) {
    const worker = system.getActor(workerId) as any;
    if (worker) {
      ...
      console.log(
        `  Circuit breaker: ${metrics.circuitBreakerState === CircuitState.CLOSED ? 'CLOSED' : metrics.circuitBreakerState}`
      );
      console.log();
